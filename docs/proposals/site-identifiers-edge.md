...
- Metrics use `site_code`, `site_city`, `site_country` from `Site.Labels()`. The "site" is fixed for the lifetime of the process.

With **edge Workers** or **Lambda** (per-request execution), there is no single config-driven site: the **location is determined by the platform at request time**. We need a clear, provider-aware way to express that location so it can be used as a site identifier in metrics and dashboards.
...
1. **Standardize on a composite label** in metrics (and optionally in config later):
   - `provider` – `technician` | `cloudflare` | `aws` (and maybe `aws-edge` to distinguish Edge from regional Lambda).
   - `site_code` – For static: config `code`. For Cloudflare: colo (IATA). For AWS regional: `AWS_REGION`. For Lambda@Edge: pop prefix or full pop code.
   So edge probes would set e.g. `provider=cloudflare`, `site_code=IAD`; no need for a pre-defined `sites` entry for every colo.
...
2. **Optional mapping layer:** If we want human-friendly names in dashboards (city, country) for edge sites, we can:
   ...
   - Allow **optional** site definitions in config that match provider + location (e.g. `provider: cloudflare`, `code: IAD`) and fill in city/country for labels when present; otherwise use raw `site_code`.

3. **Uniqueness:** For aggregation and alerting, `(provider, site_code)` should uniquely identify the execution location. So `site_code` alone is enough if we always set `provider`; or we can encode provider in the code (e.g. `cf:IAD`) and keep a single `site_code` label.
...
4. **Contract for Worker/Lambda adapters:** When implementing the blackbox-style probe endpoint in a Worker or Lambda, the adapter must:
   ...
   - **Emit** metrics (e.g. Prometheus exposition) with `site_code` (and optionally `provider`) set to that value, so scrapers and Grafana see a consistent notion of "site" across Technician binary, Cloudflare, and AWS.

## Summary

- **Cloudflare:** Use `request.cf.colo` (IATA code). No need to pre-define every colo in config; use it directly as `site_code` with `provider=cloudflare`.
- **AWS Lambda (regional):** Use `AWS_REGION`; one deployment per region if you want multiple "sites." Set `site_code` (and `provider=aws`) from the env.
- **Lambda@Edge:** Use pop code (e.g. from `x-amz-cf-pop`) or its airport prefix; set `site_code` (and `provider=aws-edge` or `aws`) so each edge location is distinguishable.
