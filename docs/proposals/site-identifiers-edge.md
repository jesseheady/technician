...
- One **site** per long-lived instance (VM, Docker, future Lambda container). The instance is started with `ORIGIN_ID=us-east-1` (or `--origin us-east-1`).
...
1. **Standardize on a composite label** in metrics (and optionally in config later):
   - `platform` – `technician` | `cloudflare` | `aws` (and maybe `aws-edge` to distinguish Edge from regional Lambda).
   ...
   So edge checks would set e.g. `platform=cloudflare`, `region=IAD`; no need for a pre-defined `origins` entry for every colo.
...
2. **Optional mapping layer:** If we want human-friendly names in dashboards (city, country) for edge sites, we can:
   ...
   - Allow **optional** origin definitions in config that match platform + location (e.g. `platform: cloudflare`, `code: IAD`) and fill in city/country for labels when present; otherwise use raw `region`.

3. **Uniqueness:** For aggregation and alerting, `(platform, region)` should uniquely identify the execution location. So `region` alone is enough if we always set `platform`; or we can encode provider in the code (e.g. `cf:IAD`) and keep a single `region` label.
...
4. **Contract for Worker/Lambda adapters:** When implementing the blackbox-style check endpoint in a Worker or Lambda, the adapter must:
   ...
   - **Emit** metrics (e.g. Prometheus exposition) with `region` (and optionally `platform`) set to that value, so scrapers and Grafana see a consistent notion of "site" across Technician binary, Cloudflare, and AWS.

## Summary

- **Cloudflare:** Use `request.cf.colo` (IATA code). No need to pre-define every colo in config; use it directly as `region` with `platform=cloudflare`.
- **AWS Lambda (regional):** Use `AWS_REGION`; one deployment per region if you want multiple "sites." Set `region` (and `platform=aws`) from the env.
- **Lambda@Edge:** Use pop code (e.g. from `x-amz-cf-pop`) or its airport prefix; set `region` (and `platform=aws-edge` or `aws`) so each edge location is distinguishable.
- **Static (current):** Keep `origins` in config and `ORIGIN_ID`; no change for existing deployments.
