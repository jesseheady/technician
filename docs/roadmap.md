...
Everything downstream of the browser handle is unchanged — device emulation, network throttling, HAR capture, and Web Vitals all operate on the context and page, not on how the browser started — so managed mode reuses the whole existing probe path. `mode: local` remains the default, `max_browsers` still bounds concurrent sessions, and an invalid mode or a managed mode without a `server_url` is rejected at startup rather than failing once per browser check. In Compose it is the default and needs no overlay: the base `docker-compose.yml` runs the `playwright` service and passes `PLAYWRIGHT_MODE` and `PLAYWRIGHT_SERVER_URL` to the worker, which `examples/technician.yml` reads as `${PLAYWRIGHT_MODE:-local}`. A config that writes `mode: local` literally ignores those variables and launches the browser in the worker while the sidecar idles, so a config tree seeded before this change keeps the old behaviour until it adopts the `${...}` form. Helm ships an optional sidecar instead (`playwright.sidecar.enabled`); with it enabled `shareProcessNamespace` is dropped, since Chromium children belong to the sidecar's PID 1.
