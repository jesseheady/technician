...
func TestTLSCheckerDefaultPort(t *testing.T) {
	...
	// but the checker should handle it gracefully
	checker := NewTLSChecker()
	cfg := &config.CheckConfig{
	...
	defer cancel()
	result := checker.Run(ctx, cfg, nil)
...
func TestTLSCheckerType(t *testing.T) {
	checker := NewTLSChecker()
	if checker.Type() != config.CheckTypeTLS {
		t.Errorf("expected type %q, got %q", config.CheckTypeTLS, checker.Type())
	}
}
